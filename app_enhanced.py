...
def main():
    ...
    st.markdown('<h1 class="main-header">📊 Enhanced Text-to-SQL Generator with Auto-Visualization</h1>', unsafe_allow_html=True)
    ...
    with col2:
        auto_viz = st.checkbox("Enable auto-visualization", value=True)
    ...
    if question.strip():
        ...
        with approach_cols[3]:
            st.markdown('<div class="approach-card">', unsafe_allow_html=True)
            st.markdown("### 🎨 Auto-Viz Mode")
            st.write("Best approach + automatic charts")
            if st.button("Auto-Visualize", use_container_width=True, key="btn_auto_viz", disabled=st.session_state.processing):
                if auto_viz:
                    st.session_state.processing = True
                    st.session_state.last_query = question
                    st.session_state.last_approach = "auto_viz"
                    st.rerun()
                else:
                    st.warning("⚠️ Please enable auto-visualization option above")
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        if st.session_state.processing and st.session_state.last_query:
            ...
            try:
                ...
                elif approach == "auto_viz":
                    st.info("🔄 Processing with Auto-Visualization...")
                    process_query_enhanced(query_to_process, show_intermediate)
